...
class Base:
    ...
    def generate_labels_syn(self, data):
        ...
        for ix, (c, num) in enumerate(sorted_counter):
            if ix == len(sorted_counter):
                num_class_dict[c] = self.args.ipc - sum_
                self.syn_class_indices[c] = [len(labels_syn), len(labels_syn) + num_class_dict[c]]
                labels_syn += [c] * num_class_dict[c]
            else:
                num_class_dict[c] = max(self.args.ipc, 1)
...
class Herding(Base):
    ...
    def select(self, embeds, inductive=True):
        num_class_dict = self.num_class_dict
        if inductive:
            idx_train = np.arange(len(self.data.feat_train))
        else:
            idx_train = self.data.idx_train
        labels_train = self.data.labels_train
        idx_selected = []
        ...
        for class_id, cnt in num_class_dict.items():
            ...
            for i in range(cnt):
                ...
                # print("idx : {}, second : {}, idx_left: {}".format(idx, features[idx_left], idx_left))
